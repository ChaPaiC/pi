...
# test for aloha training
    TrainConfig(
        name="pi0_aloha_test",#这个是你训练模型的名称
        model=pi0.Pi0Config(),
        batch_size=32,
        data=LeRobotAlohaDataConfig(
            repo_id="le",#这个是你数据集名称。
            assets=AssetsConfig(
                assets_dir="s3://openpi-assets/checkpoints/pi0_base/assets",
                asset_id="trossen",
            ),
            ...
            base_config=DataConfig(
                ...
                prompt_from_task=True, #prompt_from_task=True这个是因为之前在生成lerobot中，直接将instruction写入到每个episode_name了。
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=20_000,
